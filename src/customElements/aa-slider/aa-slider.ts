...
export class AASlider extends AABaseElement {
    ...
    inputItem: any;
    minLabelItem: HTMLElement | null = null;
    maxLabelItem: HTMLElement | null = null;
    root: ShadowRoot;
    ...
    get html(): string {
        let inputElement = customElements.get('sl-range')
            ? `<sl-range style="width:100%" class="inputItem" min="${this.min}" max="${this.max}" value="${(this.max + this.min) / 2}"></sl-range>`
